...
const buscaId = (id)=> {
    let idBusca = historicoInflacao.find(idx => idx.id === id);
...
const buscaAno = (ano) =>{
    let anoHistorico = parseInt(ano);
    let anoBusca = historicoInflacao.filter(anox => anox.ano === anoHistorico);

    return anoBusca; 
};


export const calculaIPCA = (valor, dataInicialMes, dataInicialAno, dataFinalMes, dataFinalAno) =>{
